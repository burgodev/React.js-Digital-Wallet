...
const ModalEvaluate = ({ classes, item, onClose }) => {
  const [content, setContent] = useState(CONTENT.EVALUATE);
  ...
  return (
    <Card className={classes.card}>
      ...
      {content === CONTENT.EVALUATE && (
        <Evaluate onClose={onClose} showReprove={() => setContent(CONTENT.REPROVE)} item={item} />
      )}
      {content === CONTENT.REPROVE && (
        <Reprove onClose={onClose} item={item} />
      )}
    </Card>
  );
};
